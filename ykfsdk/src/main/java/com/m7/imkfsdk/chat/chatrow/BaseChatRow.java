        ...
        if (baseHolder.getChattingAvatar() != null) {
            ...
            if ("1".equals(detail.userType)) {
                ...
                if (detail.showHtml != null && detail.showHtml) {//机器人头像
                    SharedPreferences xiaomoRobotAvator = context.getSharedPreferences(YKFConstants.MOOR_SP, 0);
                    String xiaomoRobotAvator1 = xiaomoRobotAvator.getString(YKFConstants.XIAOMO_ROBOT_AVATOR, "");
                    if (!"".equals(xiaomoRobotAvator1)) {
                        GlideUtil.loadHeader(context,xiaomoRobotAvator1 + "?imageView2/0/w/200/h/200/q/90",R.drawable.ykfsdk_kf_head_default_robot,R.drawable.ykfsdk_kf_head_default_robot,baseHolder.getChattingAvatar());
                    } else {
                    ...
                    GlideUtil.loadHeader(context,imgPath + "?imageView2/0/w/100/h/100",R.drawable.ykfsdk_kf_head_default_local,R.drawable.ykfsdk_kf_head_default_local,baseHolder.getChattingAvatar());

                } else if (!TextUtils.isEmpty(detail.user) && "system".equals(detail.user)) {
                    SharedPreferences spData = context.getSharedPreferences(YKFConstants.MOOR_SP, 0);
                    String systemMsgLogo = spData.getString(YKFConstants.SYSTEMMSGLOGO, "");
                    if (!TextUtils.isEmpty(systemMsgLogo)) {
                        GlideUtil.loadHeader(context,systemMsgLogo + "?imageView2/0/w/100/h/100",R.drawable.ykfsdk_kf_head_default_local,R.drawable.ykfsdk_kf_head_default_local,baseHolder.getChattingAvatar());
                    } else {
                        /*
                         * else : 显示默认头像
                         */
