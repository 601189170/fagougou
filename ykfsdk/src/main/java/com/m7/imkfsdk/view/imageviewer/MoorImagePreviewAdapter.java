        ...
        if (!MoorImageUtil.isGifImageWithMime(url, url)) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        final File resource = target.get();
...
//            MoorManager.getInstance().downloadImage(url, new IMoorImageLoaderListener() {
//                @Override
...
//            MoorManager.getInstance().loadGifImage(url, imageGif);
            final RequestOptions options = new RequestOptions()
                    .fitCenter()
                    .error(R.drawable.ykfsdk_pic_thumb_bg);
...
//            MoorManager.getInstance().loadImage(url, 0, 0, new IMoorImageLoaderListener() {
//                @Override
//                public void onLoadStarted(@Nullable Drawable placeholder) {
//                }
//
...
//            MoorManager.getInstance().loadGifImage(url, imageGif);
    ...
    private void setImageSpec(final String imagePath, final MoorSubsamplingScaleImageView imageView) {
        ...
        if (isLongImage) {
            imageView.setMinimumScaleType(MoorSubsamplingScaleImageView.SCALE_TYPE_START);
            ...
            imageView.setDoubleTapZoomScale(MoorImageUtil.getLongImageMaxScale(activity, imagePath));
        } else {
            ...
            if (isWideImage) {
                ...
                imageView.setMinScale(MoorImagePreview.getInstance().getMinScale());
                imageView.setMaxScale(MoorImagePreview.getInstance().getMaxScale());
                imageView.setDoubleTapZoomScale(MoorImageUtil.getWideImageDoubleScale(activity, imagePath));
